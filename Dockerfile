...
ENV PATH /opt/wildfly/bin:/usr/local/java/jdk1.8.0_102/bin:$PATH
...
WORKDIR /tmp
RUN curl -O  ftp://middleware.hostedftp.com/~binaries/wildfly10/keycloak-overlay-1.9.4.Final.tar.gz && tar -zxvf keycloak-overlay-1.9.4.Final.tar.gz -C /opt/keycloakoverlay && rm keycloak-overlay-1.9.4.Final.tar.gz

WORKDIR /opt
RUN  curl -O ftp://middleware.hostedftp.com/~binaries/wildfly10/wildfly-10.0.0.Final.tar.gz && tar -zxvf wildfly-10.0.0.Final.tar.gz  && rm wildfly-10.0.0.Final.tar.gz && mv wildfly-10.0.0.Final wildfly

WORKDIR /tmp
RUN curl -O ftp://middleware.hostedftp.com/~binaries/wildfly10/jdk-8u102-linux-x64.tar.gz  && tar -zxvf jdk-8u102-linux-x64.tar.gz -C /usr/local/java/ && rm jdk-8u102-linux-x64.tar.gz
...
RUN echo "JAVA_HOME=/usr/local/java/jdk1.8.0_102" >> /etc/profile
...
RUN update-alternatives --install "/usr/bin/java" "java" "/usr/local/java/jdk1.8.0_102/jre/bin/java" 1

RUN update-alternatives --set java /usr/local/java/jdk1.8.0_102/jre/bin/java
